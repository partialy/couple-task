...
public class AesEncryptUtils {
    ...
    public static class EncResult {
        private String randomKey; // 随机生成的 128 位十六进制字符串
     ...
     *
     * @param fullRandomKey 128 位的十六进制密钥字符串
    ...
    private static String performEncrypt(String hexKey, String plainText) {
        try {
            ...
            // 将初始化向量 (IV) 和加密数据拼接在一起，然后进行 Base64URL 编码
    ...
    /**
     * 按照指定规则从 128 位十六进制字符串中提取 32 位十六进制字符串作为 AES 密钥
    ...
    private static String extractKeyFromFullKey(String fullKey) {
        if (fullKey.length() != 128) {
            throw new IllegalArgumentException("密钥长度必须为 128 个字符。");
        }
