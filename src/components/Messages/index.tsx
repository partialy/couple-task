...
export default function Messages({ currentUser }: { currentUser: string | null }) {
  const [activeConversationId, setActiveConversationId] = useState<string | null>(null);
  const { conversations, setConversations, clearUnreadCount } = useMessageStore();
  ...
  return (
    <div className="w-full h-full relative overflow-hidden flex flex-col">
      ...
      <div className="px-4 pt-4 pb-3 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white">消息</h2>
      </div>
