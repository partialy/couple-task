...
export default function RecordTab() {
  const [records, setRecords] = useState([
    { id: 1, name: '免做家务卡', type: 'prop', count: 1, code: 'A8B9C2D4', status: 'used', date: '2026-03-12 14:30' },
    { id: 2, name: '积分奖励', type: 'points', count: 500, code: 'X9Y8Z7W6', status: 'unused', date: '2026-03-11 09:15' },
    { id: 3, name: '按摩体验券', type: 'special', count: 1, code: 'M1N2O3P4', status: 'voided', date: '2026-03-10 20:00' },
  ]);
  ...
  const handleStatusClick = (record: any) => {
    if (record.status === 'unused') {
      setConfirmModal({
        ...
        onConfirm: () => {
          setRecords(records.map(r => r.id === record.id ? { ...r, status: 'voided' } : r));
      ...
      setConfirmModal({
        ...
        onConfirm: () => {
          setRecords(records.map(r => r.id === record.id ? { ...r, status: 'unused' } : r));
          setConfirmModal(prev => ({ ...prev, isOpen: false }));
          showToast('已启用');
        }
      });
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col space-y-3 mb-4">
        <div className="flex items-center space-x-2 overflow-x-auto no-scrollbar pb-1">
          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 whitespace-nowrap mr-1">类型:</span>
          {['all', 'prop', 'points', 'special'].map(type => (
            ...
            >
              {type === 'all' ? '全部' : type === 'prop' ? '道具' : type === 'points' ? '积分' : '特殊'}
            </button>
          ))}
        </div>
        <div className="flex items-center space-x-2 overflow-x-auto no-scrollbar pb-1">
          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 whitespace-nowrap mr-1">状态:</span>
          {['all', 'unused', 'used', 'voided'].map(status => (
            ...
            >
              {status === 'all' ? '全部' : status === 'unused' ? '未兑换' : status === 'used' ? '已兑换' : '已作废'}
            </button>
          ))}
        </div>
      </div>
      ...
      {filteredRecords.length > 0 ? filteredRecords.map((record) => (
        <div key={record.id} className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-700/50 flex items-start space-x-4">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${
            record.type === 'prop' ? 'bg-purple-100 text-purple-500 dark:bg-purple-900/30' :
            record.type === 'points' ? 'bg-amber-100 text-amber-500 dark:bg-amber-900/30' :
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start mb-3">
              <h4 className="font-bold text-slate-800 dark:text-white truncate">{record.name}</h4>
              <span className="font-bold text-slate-800 dark:text-white ml-2 flex-shrink-0">
                {record.type === 'points' ? `+${record.count}` : `x${record.count}`}
              </span>
            </div>
            
            <div className="flex items-end justify-between">
              <div className="flex flex-col space-y-1.5">
                <span className="font-mono text-sm font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded w-fit tracking-wider">{record.code}</span>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div 
                  ...
                  className={record.status !== 'used' ? 'cursor-pointer hover:opacity-80 transition-opacity' : ''}
                >
                  {record.status === 'used' ? (
