...
const navItems = [
  ...
  { icon: ShoppingBag, label: "商城与兑换", path: "/shop" },
  { icon: CreditCard, label: "交易流水", path: "/transactions" },
  { icon: FileText, label: "内容治理", path: "/content" },
  { icon: MessageSquare, label: "反馈工单", path: "/feedback" },
  { icon: Settings, label: "系统配置", path: "/system" },
];
  ...
  return (
    <div className="h-screen bg-theme-gradient relative overflow-hidden flex">
      ...
      <motion.aside
        ...
        className="glass-panel border-r-0 hidden md:flex md:flex-col z-20 m-4 rounded-[24px] overflow-hidden shrink-0"
      >
        ...
        <div className="flex-1 py-6 px-3 space-y-1 overflow-y-auto custom-scrollbar">
          {navItems.map((item, index) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`w-5 h-5 shrink-0 ${isActive ? "text-white" : "text-[var(--text-muted)] group-hover:text-[var(--text-main)]"}`}
                />
                {isSidebarOpen && (
                  <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="ml-3 text-sm font-medium whitespace-nowrap">
                    {item.label}
                  </motion.span>
                )}
              </button>
        ...
        <header className="h-16 glass-panel rounded-[24px] mx-4 mt-4 flex items-center justify-between px-4 lg:px-8 z-10 shrink-0">
          <div className="flex items-center">
            ...
            <div className="relative w-64 hidden sm:block">
              ...
              <Input placeholder="搜索..." className="pl-9 input-glass h-9 rounded-full text-sm" readOnly />
            </div>
          </div>

          <div className="flex items-center space-x-4">
            <Button variant="ghost" size="icon" type="button" className="relative text-neutral-500 hover:text-neutral-900">
              ...
              <DropdownMenuContent className="w-56" align="end" forceMount>
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    ...
                    <p className="text-xs leading-none text-muted-foreground">后台账号</p>
                  </div>
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
